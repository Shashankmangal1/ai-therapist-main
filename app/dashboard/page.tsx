...
const saveMoodData = async (data: { userId: string; mood: number; note: string }) => {
  ...
  if (!res.ok) throw new Error("Failed to save mood data");
  return res.json();
};
...
export default function Dashboard() {
  ...
  const fetchDailyStats = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.warn("Error fetching activities, using empty array:", error);
      }

      // Note: Activities don't have moodScore - mood data is stored separately
      // For now, we'll set moodScore to null and it can be fetched from mood API if needed
      const averageMood = null;
  ...
  // Fetch stats on mount and every 5 minutes, but only if user is authenticated
  useEffect(() => {
