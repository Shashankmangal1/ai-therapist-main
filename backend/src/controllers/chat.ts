...
import { InngestEvent } from "../customTypes/inngest";
import { Types } from "mongoose";

// Initialize Gemini API (require key from environment)
const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
if (!GEMINI_API_KEY) {
  throw new Error(
    "GEMINI_API_KEY environment variable is required. Set it in your .env or environment."
  );
}
...
export const getAllChatSessions = async (req: Request, res: Response) => {
  try {
    ...
    const formattedSessions = sessions.map((session: any) => ({
      ...
      updatedAt: session.startTime || new Date(),
      status: session.status,
      messages: [], // Don't include messages in list view
    }));

    logger.info(`Found ${formattedSessions.length} sessions for user`);
